...
class Geometricus:
    ...
    def from_protein_files(cls,
        ...
        Geometricus object
        """
        invariants, errors = get_invariants_for_structures(input_files, split_infos, moment_types, n_threads, verbose)
        return cls.from_invariants(invariants, model=model, resolution=resolution)
    ...
    def from_invariants(
        ...
        resolution
            ...
            (This is for the old way of binning shapemers)
        """
        assert model or resolution , "Must provide either a model or resolution"
        
        if isinstance(resolution, np.ndarray):
            assert resolution.shape[0] == invariants[0].invariants[0].moments.shape[1]
        invariants: Dict[ProteinKey, MultipleMomentInvariants] = {x.name: x for x in invariants}
        
        if not protein_keys:
            protein_keys: List[ProteinKey] = list(invariants.keys())
        assert all(k in invariants for k in protein_keys)

        model = model if model else resolution
        proteins_to_shapemers = {k: invariants[k].get_shapemers_model(model) for k in tqdm(protein_keys, total=len(protein_keys))}
        ...
        geometricus_class.shapemer_keys = sorted(geometricus_class.shapemer_to_protein_indices.keys())
    ...
    def map_shapemers_to_indices(self, protein_keys=None):
        ...
        shapemer_to_protein_indices: Dict[Shapemer, List[Tuple[ProteinKey, int]]] = defaultdict(list)
    ...
    def map_protein_to_shapemer_indices(self, protein_keys=None, shapemer_keys=None):
        ...
        if protein_keys and not shapemer_keys:
            shapemer_keys = sorted(self.map_shapemers_to_indices(protein_keys).keys())
        elif not protein_keys:
            protein_keys = self.protein_keys
            if not shapemer_keys:
                shapemer_keys = self.shapemer_keys
        
        shapemer_index = {k: i for i, k in enumerate(shapemer_keys)}
        
        return {
                   k: np.array([shapemer_index[x] for x in self.proteins_to_shapemers[k] if x in shapemer_index], dtype=int)
                   for k in protein_keys
        }, shapemer_keys

    def map_shapemer_to_residues(self, shapemer: Shapemer) -> Dict[ProteinKey, Set[int]]:
    ...
    def get_count_matrix(self, protein_keys=None, shapemer_keys=None):
        ...
        return make_count_matrix([proteins_to_shapemer_indices[k] for k in protein_keys], len(shapemer_keys))
